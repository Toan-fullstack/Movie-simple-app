...
const RefEx = () => {
  ...
  const querryDebounce = useDebounce(query, 500);
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    setLoading(true);
    async function fetchData() {
      const response = await axios.get(
        `https://api.themoviedb.org/3/search/movie?api_key=95f2419536f533cdaa1dadf83c606027&query='${querryDebounce}'`
      );
      setMovies(response.data.results);
      if (response.data.results) setLoading(false);
    }
    fetchData();
  }, [querryDebounce]);
    ...
    <div className="bg-white p-3 rounded-2xl shadow-sm flex flex-col">
      <div className="h-[297px] ">
        <img
          ...
          alt=""
          className="object-center w-full h-full rounded-lg object-cover "
        />
      </div>
